...
dirichlet_func <- partial(dirichlet_reduced_mix, contaminant_prob = p_contam, alpha_indices = mix_counts, min_rt = min_rt, max_rt = max_rt, shared=TRUE)
...
sampler <- run_stage(sampler, stage = "burn", iter = 100, particles = 500, epsilon = 0.8, n_cores = cores)
save.image(outfile)
sampler <- run_stage(sampler, stage = "burn", iter = 150, particles = 500, epsilon = 0.6, n_cores = cores)
save.image(outfile)
sampler <- run_stage(sampler, stage = "burn", iter = 250, particles = 500, epsilon = 0.4, n_cores = cores)
save.image(outfile)
sampler <- run_stage(sampler, stage = "burn", iter = 4500, particles = 500, n_cores = cores)
save.image(outfile)
